...
const userImageCollection = database
  .db(mongodb_database)
  ...
  stream = cloudinary.uploader.upload(
    ...
    async function (result) {
      try {
        ...
        const success = await petCollection.updateOne(
  ...
  stream = cloudinary.uploader.upload(
    ...
    async function (result) {
      try {
        ...
        const success = await userImageCollection.updateOne(
...
router.get("/showProfile", async (req, res) => {
  try {
    ...
    if (users === null) {
      ...
      res.render("profile", { thisUser: user_id, allUsers: users });
    }
  } catch (ex) {
    res.render("error", { message: "Error connecting to MongoDB" });
    console.log("Error connecting to MongoDB");
...
router.get("/deleteUser", async (req, res) => {
  try {
    ...
    if (user_id) {
      console.log("userId: " + user_id);
      const result1 = await petCollection.deleteMany({
        user_id: new ObjectId(user_id),
      });
      const result2 = await userCollection.deleteOne({
        _id: new ObjectId(user_id),
      });
